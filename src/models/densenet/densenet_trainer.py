...
class Trainer:
    ...
    def _iteration(self, dataloader, epoch, mode="Test", sex="Overall"):
        ...
        for data, targets in tqdm(dataloader, desc="Training", leave=False):
            ...
            error_ = mean_absolute_error(
                target.detach().cpu().numpy(), output.detach().cpu().numpy()
            )
            error.update(error_)
            mape_ = mean_absolute_percentage_error(
                target.detach().cpu().numpy(), output.detach().cpu().numpy()
            )
            mape.update(mape_)
    ...
    def test(self, dataloader, epoch=None, mode="Test", sex="Overall"):
        ...
        with torch.no_grad():
            mode, t, loss, error, mape = self._iteration(
                dataloader, epoch=epoch, mode=mode, sex=sex
            )
            if mode == "Test":
    ...
    def Loop(self, epochs, trainloader, testloader, scheduler=None):
        for epoch in tqdm(range(1, epochs + 1), desc="Epochs"):
            ...
            self.save_statistic(
                *((epoch,) + self.train(trainloader, epoch=epoch, mode="Train"))
            )

            val_metrics = self.test(testloader, epoch=epoch, mode="Val")
            self.save_statistic(epoch, *val_metrics)
            print()

            # Scheduler Step
            if scheduler:
                if isinstance(scheduler, optim.lr_scheduler.ReduceLROnPlateau):
                    val_loss = val_metrics[2]
    ...
    def save_checkpoint(self, state=None, epoch=0, mode="noraml", **kwargs):
        if self.save_dir:
            ...
            if mode == "normal":
                torch.save(
                    state, os.path.join(self.save_dir, "model_epoch_{}.ckpt".format(epoch))
                )
            elif mode == "best":
    ...
    def load(self, model_pth):
        ...
        pred_optimizer_dict = {
            k: v for k, v in pred_optimizer_dict.items() if k in optimizer_dict
        }
